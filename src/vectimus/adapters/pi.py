"""Pi coding agent hook normaliser.

Translates Pi tool_call payloads into canonical VectimusEvent objects.
Pi uses lowercase tool names (bash, read, write, edit) and a similar
payload shape to Codex CLI.
"""
...
@register("pi")
def _normalise_pi(payload: dict[str, Any]) -> VectimusEvent:
    """Normalise a Pi coding agent hook payload."""
    tool_name: str = payload.get("tool_name", "unknown")
    tool_input: dict[str, Any] = payload.get("tool_input", {})
    hook_event: str = payload.get("hook_event_name", "PreToolUse")

    action_type = PI_TOOL_MAP.get(tool_name, ActionType.SHELL_COMMAND)
