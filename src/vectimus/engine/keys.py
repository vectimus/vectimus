...
def load_verify_key(key_id: str, search_dirs: list[Path] | None = None) -> VerifyKey:
    ...
    raise FileNotFoundError(f"Public key '{key_id}' not found in {[str(d) for d in search]}")
...
def _decode_ed25519_private_pem(pem: str) -> SigningKey:
    """Decode a Vectimus-format PEM-encoded Ed25519 private key (raw 32-byte body)."""
    lines = pem.strip().splitlines()
    b64 = "".join(line for line in lines if not line.startswith("-----"))
    raw = base64.b64decode(b64)
    if len(raw) != 32:
        raise ValueError(
            f"Expected a 32-byte Ed25519 private key, got {len(raw)} bytes. "
            "Standard OpenSSL PEM files are not compatible. Use a key "
            "generated by `vectimus init`."
        )
...
def _decode_ed25519_public_pem(pem: str) -> VerifyKey:
    """Decode a Vectimus-format PEM-encoded Ed25519 public key (raw 32-byte body)."""
    lines = pem.strip().splitlines()
    b64 = "".join(line for line in lines if not line.startswith("-----"))
    raw = base64.b64decode(b64)
    if len(raw) != 32:
        raise ValueError(
            f"Expected a 32-byte Ed25519 public key, got {len(raw)} bytes. "
            "Standard OpenSSL PEM files are not compatible. Use a key "
            "generated by `vectimus init`."
        )
