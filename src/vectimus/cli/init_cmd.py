...
_TOOL_CONFIG: dict[ToolName, tuple[str, str]] = {
    ...
    ToolName.OPENCODE: ("OpenCode", "_configure_opencode"),
}
...
def init_cmd(server_url: str | None, policy_dir: str | None, allow_mcp: bool) -> None:
    ...
    configure_fns = {
        ...
        ToolName.OPENCODE: _configure_opencode,
    }
...
def _configure_opencode() -> None:
    """Write .opencode/plugins/vectimus.ts with Vectimus plugin, preserving existing plugins."""
    plugins_dir = Path(".opencode") / "plugins"
    plugins_dir.mkdir(parents=True, exist_ok=True)
    plugin_path = plugins_dir / "vectimus.ts"

    command = f"{_vectimus_cmd()} hook --source opencode"

    plugin_content = f'''\
// Vectimus governance plugin for OpenCode.
// Auto-generated by `vectimus init`.  Do not edit manually.
import {{ spawnSync }} from "child_process"

export const VectimusPlugin = async (ctx: any) => {{
  return {{
    "tool.execute.before": async (input: any, output: any) => {{
      const payload = JSON.stringify({{
        ...
        session_id: input.session || "unknown",
        cwd: process.cwd(),
        principal: process.env.USER || "unknown",
      }})

      const result = spawnSync(
        "sh",
        ["-c", `echo '${{payload.replace(/'/g, "'\\\\''")}}' | {command}`],
      ...
      if (result.stdout && result.stdout.trim()) {{
        try {{
          ...
          if (decision.decision === "deny") {{
            throw new Error(
              `[vectimus] blocked by policy: ${{decision.reason || "policy violation"}}`,
            )
          }}
        }} catch (err: any) {{
          if (err.message?.startsWith("[vectimus]")) throw err
        }}
      }}
    }},
  }}
...
'''
    ...
    try:
        ...
        click.echo(f"  Error writing {plugin_path}: {exc}", err=True)
        raise SystemExit(1)
